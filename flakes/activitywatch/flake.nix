  ...
  outputs = { self, naersk, nixpkgs, napalm, rust-overlay }@inputs:
    let
      ...
      naerskUnstable = let
        ...
        rust = (nixPkgsWithNightlyRust.rustChannelOf {
          date = "2021-06-30";
          channel = "nightly";
        }).rust;
      in naersk.lib.${platform}.override {
        cargo = rust;
        rustc = rust;
      };
      ...
      aw-webui = let
        ...
        '';
      in nixPkgsWithNpalm.napalm.buildPackage
      "${sources}/aw-server-rust/aw-webui" {
        nodejs = nixpkgs.legacyPackages.${platform}.nodejs-16_x;
        nativeBuildInputs = [
    ...
    in {
      ...
      apps.${platform} = {
        aw-watcher-afk = {
          type = "app";
          program =
            "${self.packages.${platform}.aw-watcher-afk}/bin/aw-watcher-afk";
        };
        aw-watcher-window = {
          type = "app";
          program = "${
              self.packages.${platform}.aw-watcher-window
            }bin/aw-watcher-window";
        };
        aw-server = {
          type = "app";
      ...
      devShell.${platform} = nixpkgs.legacyPackages.${platform}.mkShell {
        buildInputs = [
